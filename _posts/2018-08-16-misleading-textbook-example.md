...
<img src="https://barumpark.files.wordpress.com/2018/08/beta44prior-12.png" alt="beta44prior-1" width="500" height="400" class="aligncenter wp-image-1557" />
...
<p align="center">$latex  \displaystyle  \begin{aligned}
p(\theta\verty) &amp;\propto  \underbrace{\binom{10}{2}\theta^{\sum_{i=1}^{10} y_i}(1-\theta)^{10-\sum_{i=1}^{10} y_i}}_{\text{Likelihood}} \times \underbrace{\frac{\Gamma(8)}{\Gamma(4)\Gamma(4)}\theta^3 (1-\theta)^3}_{\text{Prior}} \\
...
<img src="https://barumpark.files.wordpress.com/2018/08/plp1-12.png" alt="plp1-1" width="500" height="400" class="aligncenter wp-image-1558" />

Recall that <strong>the likelihood is not a probability distribution of $ \theta  $: it is the Binomial pmf treated as a function of $ \theta  $.</strong> It will sum to one if we sum of the number of successes from $ 0  $ to $ n=10  $ with fixed $ \theta  $; but it will <em>not</em> integrate to one if we integrate the likelihood function with respect to $ \theta  $ over $ [0,1]  $ for fixed number of successes and trials. Thus, the area under the purple curve in the figure is not equal to one.

So, what people often do is to scale the likelihood such that the area under the curve becomes equal to one. This can be done by using the fact that the the expression involving $ \theta  $ in the binomial likelihood is the kernel of a Beta distribution. Using this trick (or whatever you might call it), we get the well-known plot
...
<img src="https://barumpark.files.wordpress.com/2018/08/plp2-11.png" alt="plp2-1" width="500" height="400" class="aligncenter wp-image-1559" />
...
<img src="https://barumpark.files.wordpress.com/2018/08/p2p-12.png" alt="p2p-1" width="1000" height="800" class="aligncenter wp-image-1560" />
...
<pre><code>## [1] 4.616e-01 4.078e-01 3.411e-01 2.596e-01 1.652e-01 7.123e-02 1.104e-02

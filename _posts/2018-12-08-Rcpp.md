...
Next, we define the function `row_angles`, saved as `row_angles.cpp`, which will calculate the angles between the rows of a matrix (group positions, $\xi_j, j=1,2,...,15$) viewd from the position of point in the same space (individual position, $\theta_i$):

...
Let $ \theta  $ be the parameter vector (here treated as a random variable) and suppose that it can be partitioned into $ k  $ subsets, so that $ \theta= (\theta_1,\theta_2,...,\theta_k)  $. Note that each of the $ \theta_j  $'s might be multidimensional as well. Denote the support of $ \theta_j  $ by $ \Theta_j  $ and let $ \Theta  $ be the support of $ \theta  $. Lastly, let $ p  $ the joint density of $ \theta  $, $ p_j(\theta_j\vert\cdot)  $ the <em>conditional</em> density of $ \theta_j  $, and $ p_j(\cdot)  $ be the marginal density of $ \theta_j  $. With these definitions, the Gibbs sampler can be defined as follows:

For $ t=1,2,...,T  $,

<ol>
<li>sample $ \theta_1^{t+1} \sim p_1(\theta_1\vert\theta_2^t,\theta_3^t,...,\theta_k^t)  $</li>
<li>sample $ \theta_2^{t+1} \sim p_2(\theta_2\vert\theta_1^{t+1},\theta_3^t,...,\theta_k^t)  $</li>
<li>...</li>
<li>sample $ \theta_k^{t+1} \sim p_k(\theta_k\vert\theta_1^{t+1},\theta_2^{t+1},...,\theta_{k-1}^{t+1})  $</li>
<li>set $ \theta^{(t+1)} = (\theta_1^{t+1}, \theta_2^{t+1},...,\theta_k^{t+1})^\top  $</li>
</ol>
...
To illustrate why the Gibbs sampler works, we first focus on the case where $ \theta=(\theta_1,\theta_2)  $. We establish the result that the conditional densities completely characterize the joint density, given that the integral $ \int [p_2(x\vert\theta_1)/p_1(\theta_1\vertx)]dx  $ exists. With this assumption we have

$$  \int\frac{p_2(x\vert\theta_1)}{p_1(\theta_1\vertx)}dx = \int \frac{p(x,\theta_1)/p_1(\theta_1)}{p(x,\theta_1)/p_2(x)}dx = \frac{1}{p_1(\theta_1)}\int p_2(x)dx = \frac{1}{p_1(\theta_1)}.  $$

Therefore,

$$  p(\theta_1,\theta_2) = p_2(\theta_2\vert\theta_1)p_1(\theta_1) = \frac{p_2(\theta_2\vert\theta_1)}{\int [p_2(x\vert\theta_1)/p_1(\theta_1\vertx)]dx}.  $$
...
$$  \theta_i\vert\theta_j \sim \mathcal N(\rho\theta_j, (1-\rho^2)).  $$

The Hammersley-Clifford Theorem tells us that

<p align="center">$latex \displaystyle \begin{aligned}
p(\theta_1,\theta_2) &amp;\propto \frac{p(\theta_1\vertx_2)p(\theta_2\vert\theta_1)}{p(x_1\vertx_2)p(x_2\vert\theta_1)} \\
...
<p align="center">$latex \displaystyle \begin{aligned}
\lambda &amp;\sim \text{Dir}(\alpha) \\
\mu_j &amp;\sim \mathcal N(0,\xi_0) \quad j=1,2,3,
\end{aligned} $</p>

with $ \alpha=(\alpha_1,\alpha_2,\alpha_3)  $ and where
$ \text{Dir}(\lambda\vert\alpha) = \frac{\Gamma(\sum_j\alpha_j)}{\prod_j\Gamma(\alpha_j)} \prod_j \lambda_j^{\alpha_{j-1}}  $ is the Dirichlet distribution (you can think of it as a generalization of the Beta distribution). The likelihood of the data is

$$  p(y\vert\theta) = \prod_{i=1}^n\Big[\sum_j\lambda_j \mathcal N(y_i\vert\mu_j,\sigma)\Big]  $$

Assuming that the priors are independent <em>a priori</em>, the posterior is thus

<p align="center">$latex \displaystyle \begin{aligned}
p(\theta\verty) &amp;\propto \prod_{i=1}^n\Big[\sum_j\lambda_j \mathcal N(y_i\vert\mu_j,\sigma)\Big]\text{Dir}(\lambda\vert\alpha)N(\mu_1\vert0,\xi_0)N(\mu_2\vert0,\xi_0) \\
...
Now, this looks a little bit ugly, especially the sum within the product. But we can introduce a new latent variable $ z_i  $, which indicates class membership, into the model, where $ z_i \sim \text{Categorical}(\lambda)  $, so that $ \Pr[z_i=j]=\lambda_j  $ and $ y_i\vertz_i=j \sim \mathcal N(\mu_j,\sigma)  $. Including $ z  $ into the set of parameters, we have $ \theta = (\lambda, \mu_1,\mu_2,z)  $, and the posterior will look something like

<p align="center">$latex \displaystyle
p(\theta\verty) \propto \prod_{i=1}^n\left[\lambda_{z_i} \exp\left(-\frac{(y_i-\mu_{z_i})^2}{2\sigma^2}\right)\right]\left(\prod_j \lambda_j^{\alpha_j-1}\right)\left[\prod_j \exp\left(-\frac{\mu_j^2}{2\xi_0^2}\right)\right]
$</p>
...
<p align="center">$latex \displaystyle \begin{aligned}
\Pr[z_i = j\vert\mu,\lambda,y] &amp;\propto \Pr[\mu,\lambda\vertz_i=j,y]\Pr[z_i=j\verty]\\
&=\mathcal N(y_i\vert\mu_j,\sigma) \lambda_j
\end{aligned} $</p>

which is the kernel of a categorical distribution with normalizing constant $ \sum_j \lambda_j \mathcal N(y_i\vert\mu_j,\sigma)  $. Lastly,
...
<ol>
<li>Sample $ z_i^{t+1}  $ from the discrete distribution on $ \{1,2,3\}  $, where</li>
</ol>

$$  z_i^{(t+1)} \sim \text{Cat}\left(\frac{\mathcal N(y_i\vert\mu_1^{(t)},\sigma) \lambda_1^{(t)}}{\sum_j \mathcal N(y_i\vert\mu_j^{(t)},\sigma) \lambda_j^{(t)} },\frac{\mathcal N(y_i\vert\mu_2^{(t)},\sigma) \lambda_2^{(t)}}{\sum_j \mathcal N(y_i\vert\mu_j^{(t)},\sigma) \lambda_j^{(t)} },\frac{\mathcal N(y_i\vert\mu_3^{(t)},\sigma) \lambda_3^{(t)}}{\sum_j \mathcal N(y_i\vert\mu_j^{(t)},\sigma) \lambda_j^{(t)} }\right)  $$

<ol>
<li>Sample</li>
</ol>

$$  \mu_j^{(t+1)} \sim \mathcal N\left(\frac{\frac{n_j^{(t)}\bar{y}_j^{(t)}}{\sigma^2}}{\frac{1}{\xi_0^2}+\frac{n_j^{(t)}}{\sigma^2}},\left(\frac{1}{\xi_0^2}+\frac{n_j^{(t)}}{\sigma^2}\right)^{-1/2}\right)  $$

where we note that $ n_j  $ and $ \bar y_j  $ are now random as they depend on $ z  $.

<ol>
<li>Lastly, update $ \lambda  $ by sampling from</li>
</ol>
...
geom_histogram(alpha=.5, col='white', bins=50, position='identity') +
...
rnorm(1, (n.j[w]*j.means[w]/sigma^2)/(1/xi0^2 + n.j[w]/sigma^2),
...
n.iter =15000
n.burnin = 0
res <- matrix(NA, nrow=n.iter-n.burnin, ncol=length(mu.t)+length(lambda.t))
...
setnames(df, c('iter',paste0('mu[',1:k,']'),
paste0('lambda[',1:k,']')))
...
So, it seems that the code is working! Fortunately, we have not encountered what is often referred to as <em>label switching</em>. Namely, we could label the classes as class 1, class 2, and class 3, as we did here, but we could also label them in the order 3, 2, 1: the substantive interpretation of the model would not change. Actually, all 6 permutations of the 3 class labels will result in the same likelihood (i.e., the likelihood of the model is not <em>identified</em>), so that the posterior distribution has 6 modes, one mode for each permutation of the labels. Hence, it often happens that algorithm starts with sampling from one mode but thereafter switches to another mode of the posterior distribution. Such label swtiching phenomena can be avoided by 1) using initial values close to one of the modes (as we did here) and <em>pray</em> that the Gibbs sampler will not switch to another mode, 2) using informative priors for the parameters that favor a certain mode, 3) imposing a set of hard and/or sign constraints on the paramters, or 4) post-processing the MCMC samples.
